...
import { collection, query, where, getDocs, orderBy, doc, getDoc } from 'firebase/firestore'; // Import getDoc
...
const HistoryPage = () => {
  ...
  const getSortValue = (item, key) => {
    ...
    switch (key) {
      ...
      case 'status':
        return (item.score / numQuestions) >= (item.quizType === 'interview' ? 0.66 : 0.8) ? 1 : 0; // Use numQuestions here and the correct percentage
      default:
        return 0;
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen p-6 bg-light-blue-matte dark:bg-dark-blue-matte text-light-text dark:text-white pt-20">
            ...
            {!isAuthenticated ? (
              ...
              <>
                ...
                <div className="mt-2 p-1 rounded-lg shadow-md border border-gray-300 dark:border-gray-700">
                  ...
                  />
                  {/* Chart */}
                  <HistoryChart chartData={chartData} selectedCategory={selectedCategory} />
                </div>
              </>
            )}
          </>
      ...
      {/* Feedback Popup */}
      {selectedFeedback && (
        <FeedbackPopup feedback={selectedFeedback} onClose={handleCloseFeedback} />
      )}
    </div>
  );
};
