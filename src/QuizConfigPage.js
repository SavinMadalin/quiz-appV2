...
import { ClockIcon, CpuChipIcon, Cog6ToothIcon, ChevronDownIcon, ChevronUpIcon } from '@heroicons/react/24/outline';
...
const QuizConfigPage = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen p-4 bg-light-blue-matte dark:bg-dark-blue-matte text-light-text dark:text-white pt-24 overflow-hidden">
      <TopNavbar />
      ...
      <div className="bg-white dark:bg-dark-grey p-7 rounded-lg shadow-lg max-w-md w-full mt-8">
        ...
        <div className="mb-4 border-b border-gray-300 dark:border-gray-600">
          <div className="flex w-full">
            <button
              ...
              className={`w-1/2 py-3 focus:outline-none text-base ${activeTab === 'custom' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-700 dark:text-gray-300'}`}
            >
        ...
        {activeTab === 'custom' && (
          <div>
            ...
            <div className="mb-4" ref={categoryDropdownRef}>
              <label htmlFor="category" className="block font-medium mb-2 flex items-center gap-2 text-base">
                {getCategoryIcon()}
                Category:
              </label>
              <div className="relative">
                ...
                >
                  {mainCategories.find(cat => cat.value === draftSettings.category)?.label || 'Select Category'}
                  {isCategoryDropdownOpen ? <ChevronUpIcon className="h-5 w-5" /> : <ChevronDownIcon className="h-5 w-5" />}
                </button>
            ...
            {draftSettings.category !== 'ai' && (
              <div className="mb-6">
                <div className="flex justify-between items-center">
                  <label className="block font-medium mb-2 text-base">Subcategories (multiple selection):</label>
                </div>
            ...
            <div className="mb-8" ref={timeDropdownRef}>
              <label htmlFor="timePerQuestion" className="block font-medium mb-2 flex items-center gap-2 text-base">
              ...
              <div className="relative">
                <button
                  ...
                  className="mt-1 p-2 border rounded-md bg-light-grey dark:bg-gray-800 dark:text-white w-full focus:outline-none focus:ring-2 focus:ring-blue-500 flex items-center justify-between cursor-pointer text-base"
                >
                  {timeOptions.find(option => option.value === draftSettings.timePerQuestion)?.label || 'Select Time'}
                  {isTimeDropdownOpen ? <ChevronUpIcon className="h-5 w-5" /> : <ChevronDownIcon className="h-5 w-5" />}
                </button>
                {isTimeDropdownOpen && (
                  <div className="absolute z-10 mt-1 w-full rounded-md bg-gray-100 dark:bg-gray-700 shadow-lg ring-1 ring-black ring-opacity-5">
                    <div className="py-1">
                      {timeOptions.map((option) => (
                        <button
                          key={option.value}
                          onClick={() => handleChange('timePerQuestion', option.value)}
                          className="block w-full px-4 py-2 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 border-b border-gray-300 dark:border-gray-600 last:border-b-0"
                        >
                          {option.label}
                        </button>
        ...
        {activeTab === 'interview' && (
          <div>
            ...
            {draftSettings.category !== 'ai' && (
              <div className="mb-6">
                <div className="flex justify-between items-center">
                  <label className="block font-medium mb-2 text-base">Subcategories:</label>
                </div>
        ...
        <div className="relative group">
          <button
            ...
            className={`w-full bg-green-500 ${isStartButtonDisabled || (dailyAttempts >= 2 && (!isAuthenticated || !isEmailVerified)) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-600'} text-white font-bold py-3 px-6 rounded-md shadow-md transition duration-300 ease-in-out transform hover:scale-105 block text-center text-base`}
          >
            Start
          </button>
